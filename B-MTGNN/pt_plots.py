...
model = gtnet(
    gcn_true=arch["gcn_true"],
    buildA_true=arch["buildA_true"],
    gcn_depth=arch["gcn_depth"],
    num_nodes=arch["num_nodes"],
    device='cpu',
    predefined_A=torch.eye(arch["num_nodes"]), 
...
model.eval()
model.mc_dropout = True # Enable MC Dropout during inference


# Bayesian estimation using Law of Total Variance
num_runs = 30 # Increased for better MC average stability
...
Y = torch.mean(mu_outputs, dim=0)

# Epistemic Uncertainty (Model's lack of knowledge, via MC Dropout variance)
epistemic_var = torch.var(mu_outputs, dim=0)
# Aleatoric Uncertainty (Data's inherent noise, via predicted sigma^2)
aleatoric_var = torch.mean(sigma_outputs ** 2, dim=0)

# Law of Total Variance
total_variance = epistemic_var + aleatoric_var
total_std = torch.sqrt(total_variance)

# Calculate true 95% prediction interval (No sqrt(N) division which causes artificial shrinkage!)
z = 1.96
confidence = z * total_std
